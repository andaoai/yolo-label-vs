# <img src="./images/icon.png" width="32" height="32" alt="YOLO标注工具图标"> YOLO 标注工具

[![发布到VS Code插件市场](https://github.com/andaoai/yolo-label-vs/actions/workflows/main-publish.yml/badge.svg)](https://github.com/andaoai/yolo-label-vs/actions/workflows/main-publish.yml)
[![VS插件市场版本](https://img.shields.io/badge/VS_Marketplace-v0.0.89-blue?link=https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs)](https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs)
[![VS插件市场安装量](https://img.shields.io/badge/安装量-4.5K-brightgreen?link=https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs)](https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs)
[![VS插件市场评分](https://img.shields.io/badge/评分-5.0%2F5-yellow?link=https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs)](https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs)
[![许可证: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![GitHub Stars](https://img.shields.io/github/stars/andaoai/yolo-label-vs?style=social)](https://github.com/andaoai/yolo-label-vs/stargazers)
[![GitHub Issues](https://img.shields.io/github/issues/andaoai/yolo-label-vs)](https://github.com/andaoai/yolo-label-vs/issues)
[![GitHub Pull Requests](https://img.shields.io/github/issues-pr/andaoai/yolo-label-vs)](https://github.com/andaoai/yolo-label-vs/pulls)
[![VS Code引擎版本](https://img.shields.io/badge/vscode-%5E1.85.0-blue)](https://code.visualstudio.com/)
[![Node.js版本](https://img.shields.io/badge/node-%3E%3D20.0.0-brightgreen)](https://nodejs.org/)
[![赞助支持](https://img.shields.io/badge/赞助-支持-brightgreen)](https://github.com/andaoai/yolo-label-vs/blob/main/docs/README_CN.md#赞助支持)
[![Ask DeepWiki](https://img.shields.io/badge/Ask%20DeepWiki-blue)](https://deepwiki.com/andaoai/yolo-label-vs)

一个专门用于快速浏览和编辑 YOLO 数据集标注的 VS Code 扩展。通过 YAML 配置文件，您可以直接在 VS Code 中高效地查看和修改 YOLO 格式的标注数据，让数据集管理变得轻松简单。

## 演示

![YOLO Label VS 演示](https://raw.githubusercontent.com/andaoai/yolo-label-vs/main/docs/images/demo.gif)

## 数据集统计仪表板

基于 Chart.js 的交互式数据可视化，全面展示数据集分析结果：

- **类别分布**：饼图和柱状图展示标签分布情况
- **训练/验证/测试对比**：各子集类别分布的对比图表
- **边界框统计**：尺寸直方图、宽高比分析、中心位置热力图
- **单图标签数分布**：每张图片标签数量分布直方图
- **文件夹分布**：图片在各子目录下的分布情况

![数据集统计仪表板](images/%E6%95%B0%E6%8D%AE%E5%8F%AF%E8%A7%86%E5%8C%96.png)

## 主题支持

我们的扩展无缝集成了所有 VS Code 主题，提供一致的用户体验：

<table>
  <tr>
    <td width="33%"><img src="images/themes/white.png" width="100%" alt="白色主题"></td>
    <td width="33%"><img src="images/themes/black.png" width="100%" alt="黑色主题"></td>
    <td width="33%"><img src="images/themes/blue.png" width="100%" alt="蓝色主题"></td>
  </tr>
  <tr>
    <td width="33%"><img src="images/themes/green.png" width="100%" alt="绿色主题"></td>
    <td width="33%"><img src="images/themes/red.png" width="100%" alt="红色主题"></td>
    <td width="33%"><img src="images/themes/brown.png" width="100%" alt="褐色主题"></td>
  </tr>
</table>

## 相关文档

- 中文文档
  - [打包指南](./PACKAGING_CN.md)
  - [发布指南](./PUBLISHING_CN.md)
  - [开发指南](./DEVELOPMENT_CN.md)
  - [CI/CD 工作流指南](./CI_CD_WORKFLOW_CN.md)
  - [README 中文版](./README_CN.md)
- English Documentation
  - [Packaging Guide](./PACKAGING.md)
  - [Publishing Guide](./PUBLISHING.md)
  - [Development Guide](./DEVELOPMENT.md)
  - [CI/CD Workflow Guide](./CI_CD_WORKFLOW.md)
  - [README in English](../README.md)

## 核心功能

- **AI 智能推理**：加载 ONNX 模型自动检测图像中的目标，支持置信度和 IoU 阈值调节
- **数据集资源管理器**：在 VS Code 侧边栏直接浏览数据集 - 按 YAML 配置、训练/验证/测试子集和单张图片分层展示
- **数据集统计仪表板**：可视化展示标签分布、类别统计、边界框尺寸、训练/验证/测试类别对比图表
- **快速数据集浏览**：通过 YAML 配置文件即时查看 YOLO 标注的图像
- **高效标签管理**：在 VS Code 中直接修改已有标注
- **全新侧边栏 UI**：可折叠的类别选择器（支持搜索）和交互式标签列表（带有 BOX/SEG/POSE 类型标识）
- **直观预览**：实时可视化边界框和标签
- **流畅导航**：使用快捷键快速切换图片
- **YAML 集成**：直接支持 YAML 配置文件
- **批量处理**：连续浏览和编辑多个图像
- **复制粘贴标签**：使用 Ctrl+C/Ctrl+V 快速复制标签
- **多文件夹子集支持**：可视化展示多数据源数据集，按子目录分组显示

## 支持的 YOLO 模型

目前支持的 YOLO 推理模型：

| 模型 | 任务 | 状态 |
|------|------|------|
| YOLOv8 | 检测 (det) | ✅ 已支持 |
| YOLOv8 | 分割 (seg) | ✅ 已支持 |
| YOLOv11 | 检测 (det) | ✅ 已支持 |
| YOLOv11 | 分割 (seg) | ✅ 已支持 |

> **注意**：仅支持 ONNX 格式模型。请使用 Ultralytics 将 PyTorch 模型导出为 ONNX 格式后再加载。

## 支持的数据格式

<table>
  <tr>
    <th>类别</th>
    <th>格式</th>
    <th>状态</th>
    <th>描述</th>
  </tr>
  <tr>
    <td rowspan="2"><b>目标检测</b></td>
    <td><b>COCO8</b></td>
    <td>✅ 已支持</td>
    <td>包含8张COCO图像（4训练，4验证）的小型目标检测数据集</td>
  </tr>
  <tr>
    <td><b>COCO128</b></td>
    <td>✅ 已支持</td>
    <td>COCO train2017数据集的前128张图像，用于目标检测测试</td>
  </tr>
  <tr>
    <td rowspan="1"><b>分割</b></td>
    <td><b>COCO8-seg</b></td>
    <td>✅ 已支持</td>
    <td>带有实例分割标注的8张COCO图像</td>
  </tr>
  <tr>
    <td rowspan="2"><b>姿态关键点</b></td>
    <td><b>COCO8-pose</b></td>
    <td>✅ 已支持</td>
    <td>带有关键点标注的8张COCO图像，用于姿态估计</td>
  </tr>
  <tr>
    <td><b>Tiger-pose</b></td>
    <td>✅ 已支持</td>
    <td>263张老虎图像，每只老虎有12个关键点</td>
  </tr>
  <tr>
    <td rowspan="1"><b>旋转边界框</b></td>
    <td><b>DOTA8</b></td>
    <td>✅ 已支持</td>
    <td>带有旋转边界框的8张航拍图像小型子集（使用分割模式）</td>
  </tr>
  <tr>
    <td rowspan="2"><b>分类</b></td>
    <td><b>MNIST160</b></td>
    <td>❌ 未计划</td>
    <td>MNIST每个类别的前8张图像（共160张图像）</td>
  </tr>
  <tr>
    <td><b>ImageNet-10</b></td>
    <td>❌ 未计划</td>
    <td>包含10个类别的ImageNet子集</td>
  </tr>
  <tr>
    <td rowspan="1"><b>多目标跟踪</b></td>
    <td><b>VisDrone</b></td>
    <td>❌ 未计划</td>
    <td>用于跨帧跟踪多个目标的无人机图像</td>
  </tr>
</table>

<div align="center">

# 🌟 探索更多数据集格式 🌟

**Ultralytics支持全面的数据集类型**  
目标检测 · 实例分割 · 姿态估计 · 图像分类 · 目标跟踪

[![探索数据集](https://img.shields.io/badge/Ultralytics-探索所有数据集-blue?style=for-the-badge&logo=data:image/svg+xml;base64,)](https://docs.ultralytics.com/datasets/)

**COCO · VOC · ImageNet · DOTA · 以及更多**

</div>

## 为什么选择本扩展？

- **简化工作流程**：无需在不同工具间切换 - 直接在 VS Code 中查看和编辑 YOLO 数据集
- **开发者友好**：完美适配需要快速验证或调整 YOLO 数据集的机器学习工程师
- **轻量级设计**：快速响应，专为处理大型数据集而设计
- **集成体验**：无缝融入您的开发环境

## 系统要求

- Visual Studio Code 1.85.0 或更高版本
- 工作区中的图片文件
- 用于 YOLO 标注的 YAML 配置文件

## 安装方法

1. 打开 VS Code
2. 按下 `Ctrl+P` 打开快速打开对话框
3. 输入 `ext install andaoai.yolo-labeling-vs`
4. 按回车键安装

或者您可以直接从 [VS Code 插件市场](https://marketplace.visualstudio.com/items?itemName=andaoai.yolo-labeling-vs) 安装。

## 使用方法

### 数据集资源管理器 (TreeView)

1. 在 VS Code 中打开 YOLO 数据集文件夹
2. 扩展会自动扫描所有 YAML 配置文件
3. 在 VS Code 侧边栏找到 "YOLO Datasets" 面板
4. 展开数据集以浏览 Train/Val/Test 子集和单张图片
5. 点击任意数据集、子集或图片可直接打开标注面板
6. 使用刷新按钮重新扫描数据集变更

### 统计仪表板

1. 右键点击 YAML 文件或使用数据集 TreeView
2. 选择 "查看统计" 打开数据分析仪表板
3. 可视化展示：
   - **概览**：总图片数、已标注/未标注数量、单图平均标签数
   - **类别分布**：饼图和柱状图展示标签分布情况
   - **训练/验证/测试对比**：各子集类别分布的对比图表
   - **边界框统计**：尺寸直方图、宽高比、中心位置热力图
   - **单图标签数分布**：每张图片标签数量分布直方图
   - **文件夹分布**：图片在各子目录下的分布情况

### 标注面板

1. 打开包含 YAML 配置文件和对应图片的文件夹
2. 在资源管理器中右键点击 YAML 文件
3. 选择 "打开 YOLO 标注面板"
4. 浏览已标注的图片并根据需要进行调整

### 界面控件

- **侧边栏**：可折叠的类别选择区域（支持搜索）和标签列表区域（带有类型标识）
- **上一张/下一张图片**：在数据集中导航浏览图片
- **模式选择器**：在边界框(Box)、分割(Segmentation)和姿态(Pose)标注模式之间切换
- **显示标签**：切换图像上标签的可见性
- **保存标签**：将当前标注保存到磁盘
- **搜索框**：在数据集中搜索特定图片
- **模型面板**：加载 ONNX 模型并运行 AI 推理自动检测目标
  - 加载/更换模型：选择 .onnx 模型文件
  - 置信度阈值：调整检测置信度（0.05-0.95）
  - IoU 阈值：调整非极大值抑制重叠度（0.1-0.9）
  - 运行推理：检测当前图像中的目标
  - 接受/拒绝：将检测到的目标添加为标签或丢弃

### 快捷键

#### 全局快捷键
- `Ctrl+Y`: 打开 YOLO 标注面板

#### 标注面板内快捷键
- `Tab`: 切换到下一个标签类型
- `Shift+Tab`: 切换到上一个标签类型
- `空格`：重置图像缩放和位置
- `Ctrl + 鼠标悬停`：高亮边界框、分割区域或姿态标注
- `Ctrl + 鼠标拖动`：移动高亮的标注对象
- `D`：下一张图片
- `A`：上一张图片
- `Ctrl+S`：保存标注
- `Ctrl+Z`：撤销最后的标注操作
- `Ctrl+Y`：重做上一步撤销的操作
- `Ctrl+C`：复制悬停的标签
- `Ctrl+V`：粘贴复制的标签
- `Ctrl+滚轮`：以鼠标位置为中心缩放
- `Alt+拖动`：在放大状态下平移画布
- `滚轮`：在放大状态下垂直滚动
- `Shift+滚轮`：在放大状态下水平滚动
- `Esc`：取消当前绘制操作（分割模式或姿态模式）

#### 标注模式特定快捷键
- **分割模式**：
  - `鼠标左键点击`：添加多边形点
  - `右键点击`：取消多边形绘制
  
- **姿态模式**：
  - `鼠标左键点击`：标注完全可见的关键点（可见性=1）
  - `鼠标右键点击`：标注被遮挡的关键点（可见性=2）
  - `Esc`：取消当前姿态标注

#### 搜索功能
- `向下箭头`: 在搜索结果中向下移动
- `向上箭头`: 在搜索结果中向上移动
- `回车`: 选择高亮的搜索结果
- `Esc`: 关闭搜索结果面板

## 扩展设置

此扩展提供以下命令：

* `yolo-labeling-vs.openLabelingPanel`: 打开 YOLO 标注面板
* `yolo-labeling-vs.openDatasetStats`: 打开数据集统计仪表板
* `yolo-labeling-vs.refreshDatasets`: 刷新数据集资源管理器

## 已知问题

如果您发现任何问题，请在我们的 [GitHub 仓库](https://github.com/andaoai/yolo-label-vs/issues) 上报告。

## 版本说明

有关详细的版本说明和版本历史，请参阅 [CHANGELOG.md](../CHANGELOG.md) 文件。

## 贡献代码

我们欢迎您的贡献！请随时提交 Pull Request。

## 赞助支持

如果您觉得这个扩展有帮助，可以考虑赞助支持开发：

<table>
  <tr>
    <th>微信支付</th>
    <th>支付宝</th>
  </tr>
  <tr>
    <td><img src="https://raw.githubusercontent.com/andaoai/yolo-label-vs/main/docs/images/微信支付_20250503021350.jpg" width="300" alt="微信支付"></td>
    <td><img src="https://raw.githubusercontent.com/andaoai/yolo-label-vs/main/docs/images/支付宝支付_20250503021444.jpg" width="300" alt="支付宝"></td>
  </tr>
</table>

## 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](../LICENSE) 文件。

## 支持

如果您遇到任何问题，请在我们的 [问题追踪器](https://github.com/andaoai/yolo-label-vs/issues) 上提交问题。 